...
def _quote_liquidity(symbol: str, price: float) -> dict:
    ...
    try:
        ...
        if not isinstance(q, dict):
            ...
            log.warning("DAILY QUOTE REJECT | %s | reason=%s response_type=%s",
            ...
            if not result["ok"]:
                log.warning(
                    "DAILY QUOTE REJECT | %s | reason=%s bid=%s ask=%s age=%.3fm feed=%s source=%s ts=%s",
        ...
        log.warning(
            "DAILY QUOTE EXCEPTION | %s | %s | feed=IEX",
...
def _final_execution_snapshot(symbol: str, reference_price: float, quote_snapshot: dict | None = None) -> dict:
    ...
    try:
        ...
        result["reason"] = f"exception:{type(exc).__name__}:{str(exc)[:220]}"
        log.warning("DAILY FINAL EXECUTION SNAPSHOT FAILED | %s | %s", symbol, result["reason"], exc_info=True)
    return result
