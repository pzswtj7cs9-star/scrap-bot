...
def analyze(symbol: str, name: str = "", with_live: bool = True) -> SignalResult:
    ...
    lookback_hi = df["High"].iloc[-90:] if len(df) >= 20 else df["High"]
    period_high = float(lookback_hi.max()) if len(lookback_hi) else price
...
def format_signal_ar(sig: SignalResult, min_score: int = 84, rank: int | None = None) -> str:
    ...
    if getattr(sig, "near_new_high", False):
        lines.append(
            f"قرب قمة جديدة: أعلى قمة {getattr(sig, 'period_high', 0):.2f} | المسافة {getattr(sig, 'dist_from_high_pct', 0):.2f}% (حد الإرسال 2%+)"
        )
